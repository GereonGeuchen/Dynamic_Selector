...
# === Parameters ===
ela_data_dir = "../data/lookahead_data/auc/A1_data_ela_with_future_aucs_new_normalisation"  # where the ELA files with future precision columns are located
...
untuned_models_dir = "../data/models/trained_models/auc/lookahead_models_all_epms_auc"
...
def train_default_lookahead_model(budget: int, horizon: int):
    """
    Trains a default (untuned) lookahead model for the given budget and horizon.
    Saves model to untuned_models_dir.
    """
    if horizon > (1000 - budget) // 50:
        print(f"Skipping B{budget}, t+{horizon}: horizon not available from budget.")
        return None
    ...
    model = RandomForestRegressorWrapper.get_from_configuration(default_config, random_state=42)()

    # Fit model on all data (no CV, no tuning)
    X_train, y_train, _groups = _make_X_y_groups(df, horizon=horizon)
    y_train_log = np.log10(y_train)
    model.fit(X_train, y_train_log)


    out_dir = Path(untuned_models_dir)
    out_dir.mkdir(parents=True, exist_ok=True)

    model_path = out_dir / f"lookahead_model_B{budget}_t{horizon}_untuned_trained.pkl"
    joblib.dump(model, model_path)

    print(f"Saved default untrained lookahead model for budget {budget}, t+{horizon} to {model_path}")
    return model_path
...
def main():
    ...
    for horizon in range(0, 20):
        ...
        train_default_lookahead_model(budget=budget, horizon=horizon)
