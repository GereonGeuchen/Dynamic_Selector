...
def create_selection_model_data(data_path: str, switching_budgets: list, store_data: bool = False):
    ...
    ela_features = pd.read_csv(ela_features_path(data_path, NO_SWITCH_ALGORITHM, TOTAL_BUDGET))
    ...
    for budget in switching_budgets:
        ...
        if store_data:
            ...
            budget_output_path = selection_training_data_path(data_path, budget)
...
def create_lookahead_model_data(selection_model_training_data: dict[int, pd.DataFrame], normalize_lookahead_performances: bool = True, store_final_data: bool = True, data_output_path: str = "./data") -> dict[int, pd.DataFrame]:
    ...
    for budget in switching_budgets:
        ...
        if store_final_data:
            ...
            os.makedirs(output_path, exist_ok=True)
            budget_output_path = lookahead_training_data_path(data_output_path, budget)
...
class DynamicSelector:
    ...
    def simulate_single_run(self, fid: int, iid: int, rep: int, ela_features_instance: pd.DataFrame, metrics: pd.DataFrame) -> dict:
        """
        ...
        for fid in range(1, 25):
            for iid in TEST_IIDS:
                for rep in range(0, 20):
                    ...
                    result = self.simulate_single_run(fid, iid, rep, ela_features_instance, metrics)
