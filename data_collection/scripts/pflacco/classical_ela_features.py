...
def calculate_dispersion(
      ...
      medians = []
      for key in dists:
            nonzero_dists = dists[key][dists[key] != 0]
            # === Added by me to avoid empty nonzero_dists ===
            if nonzero_dists.size == 0:
                  means.append(0.0)
                  medians.append(0.0)
            # ================================================
            else:
                  means.append(np.mean(nonzero_dists))
                  medians.append(np.median(nonzero_dists))
...
def calculate_information_content(
      ...
      # Calculate psi eps
      psi_eps = []
      y_perm = np.asarray(y)[permutation]
...
def calculate_ela_level(
      ...
      for prob in ela_level_quantiles:
            ...
            y_class = (y < y_quant)
             # First check
             # === Added by me if cma converges early and y_class is empty ===
            if y_class.sum() < ela_level_resample_iterations:
                  # Retry using <= instead of <
                  print(f'Too few observations using < for quantile {prob} with {y_class.sum()} positives. Trying <=.')
                  y_class = (y <= y_quant)
                  if y_class.sum() < ela_level_resample_iterations:
                        raise Exception(f'Too few observations even after using <= for quantile {prob} with {y_class.sum()} positives.')
            # === End of added code ===========================================
